...
function loadPackageInfo() {
	...
	if (navigator.userAgent.search(/Cydia/) == -1) {
		$("#showAddRepo_").show();
		$("#open_cydia").attr("href","cydia://url/https://cydia.saurik.com/api/share#?source=https://firefoxdidong.github.io/&package="+urlSelfParts[1]);
	}
	$.ajax({
		url: form_url,
		type: "GET",
...
function openSection(sectionName)
{
	...
	for (var keyNow in packagesSection[sectionName]) {
		...
		sectionContent +=  "<li class=\"has-icon\"><a href='"+urlOpen+"' target='_blank' role=\"button\"><img style=\"border-radius: 20%;\" href=\""+encodeURI(sectionName)+".png\" alt=\"\" srcset=\""+encodeURI(sectionName)+".png 2x, "+encodeURI(sectionName)+".png 3x\" class=\"icon\"/><label>"+dicNow.name+" Phiên bản "+dicNow.version+"</label></a></li>";
	}
...
function loadRecentUpdates()
{
	...
	for (var dicNow in allPackages) {
		...
		htmlnews +=  "<li class=\"has-icon\"><a href='"+urlOpen+"' target='_blank' role=\"button\"><img style=\"border-radius: 20%;\" href=\""+encodeURI(allPackages[dicNow].section)+".png\" alt=\"\" srcset=\""+encodeURI(allPackages[dicNow].section)+".png 2x, "+encodeURI(allPackages[dicNow].section)+".png 3x\" class=\"icon\"/><label>"+allPackages[dicNow].name+" Phiên bản "+allPackages[dicNow].version+"</label></a></li>";
	}
